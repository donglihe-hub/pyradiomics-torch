...
class PyRadiomicsCommandLine:
        ...
        try:
            ...
            if caseGenerator is not None:
                if self.args.validate:
                    ...
                    self.logger.info(
                        "Finished %s-based extraction successfully...", self.args.mode
                    )
            else:
    ...
    def _validateCases(self, case_generator):
        ...
        for case_idx, case in case_generator:
            ...
            if not os.path.isfile(case["Image"]):
                case_error = True
                self.logger.error(
                    "Image path for case (%i/%i) does not exist!",
                    case_idx,
                    self.case_count,
                )
            if not os.path.isfile(case["Mask"]):
                case_error = True
                self.logger.error(
                    "Mask path for case (%i/%i) does not exist!",
                    case_idx,
                    self.case_count,
                )
        ...
        self.logger.info(
            "Validation complete, errors found in %i case(s)", errored_cases
        )
        ...
        for case_idx, case in enumerate(results, start=1):
            ...
            if self.args.format == "csv":
                writer = csv.DictWriter(
                    self.args.out, headers, lineterminator="\n", extrasaction="ignore"
                )
                if case_idx == 1:
                    writer.writeheader()
                writer.writerow(
    ...
    def _parseOverrides(self):
        ...
        for setting in self.args.setting:  # setting = "setting_key:setting_value"
            if ":" not in setting:
                self.logger.warning(
                    'Incorrect format for override setting "%s", missing ":"', setting
                )
                continue
            ...
            if setting_key not in settingsSchema:
                self.logger.warning(
                    'Did not recognize override "%s", skipping...', setting_key
                )
                continue
    ...
    def _configureLogging(self):
        ...
        logging_config = {
            ...
            "loggers": {"radiomics": {"level": logger_level, "handlers": ["console"]}},
        }
        ...
        if self.args.log_file is not None:
            ...
            if self.args.jobs > 1 and py_version >= (3, 2):
                ...
                file_handler = logging.FileHandler(
                    filename=self.args.log_file, mode="a"
                )
                file_handler.setFormatter(
                    logging.Formatter(
                        fmt=logging_config["formatters"]["default"].get("format"),
